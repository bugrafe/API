...
import { getAllUsers, getUserById,deleteUser,createUser,updateUser } from "../user-api.js";
...
const renderUserList = (users) => {
    ...
    users.forEach( (item,index) => {
        strUsers+=` <tr>
                        ...
                        <td>${item.lastName}</td>
                        <td>${item.email}</td>
                        <td style="width:100px; text-nowrap"><button class="btn btn-info btn-sm btn-edit" data-id="${item.id}">✍️</button></td>
                        <td><button class="btn btn-danger btn-sm btn-del" data-id="${item.id}">🗑</button></td>
